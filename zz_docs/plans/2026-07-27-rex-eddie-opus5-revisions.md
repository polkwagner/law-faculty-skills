...
2. *The obvious replacements were the scrub's own output.* `expected.md` and the published fixtures in the repo refer to "Maya Calloway" and "Diane Holloway" — but those names exist **nowhere in source**. They are what the private scrub *generates* (`Maya Calloway → Maya Calloway`, `Diane Holloway → Diane Holloway`). Putting them in a source file yields two `Maya Calloway` entries in the published registry. **When choosing fixture names, read the source tree under `~/.claude/skills/eddie/`, never the published copy in the repo, and check candidates against the replacement column of `~/.claude/publish-private-scrub.json`.**

- [ ] **Step 1b: Register the two principals**

Add to `~/.claude/skills/eddie/tests/NAMES.md` under the existing `## People` heading, matching the formatting of the entries already there. Leave the `Maya Calloway` and `Diane Holloway` entries and the known-fabrications table untouched.
